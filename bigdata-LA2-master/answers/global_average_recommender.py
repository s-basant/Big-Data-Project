...
avg_ratings = model.transform(test).groupBy().avg('rating').collect()[0][0]
new_ratings = ratings.withColumn('avg_ratings', lit(avg_ratings))
(training, test) = new_ratings.randomSplit([0.8, 0.2], seed = int(sys.argv[1]))
als = ALS(maxIter=5, regParam=0.01, userCol="userId", itemCol="movieId", ratingCol="avg_ratings",
